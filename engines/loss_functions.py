...
class MSEWithVarianceLoss(torch.nn.Module):
    """
    Custom loss function that combines Mean Squared Error (MSE) loss 
    with a variance-promoting term. The goal is to encourage predictions 
    that not only match the targets but also have greater spread (variance), 
    which can be useful in certain tasks like ensemble diversity or exploration.

    Args:
        lambda_var (float): Weighting factor for the variance term. 
                            Controls the influence of variance in the loss.
    """
    def __init__(self, lambda_var=3.67e-5):
        super().__init__()
        self.mse = torch.nn.MSELoss()
        self.lambda_var = lambda_var  # Weight for variance regularization
    ...
    def forward(self, predictions, targets):
        ...
        # Encourage variance by subtracting it (scaled) from the MSE loss
        variance_penalty = torch.clamp(self.lambda_var * variance, max=0.99 * mse_loss)

        # Final loss: MSE minus the scaled variance (to encourage spread)
