...
class RegressionEngine(Common):
    ...
    def init_train(
        ...
        ordinary_regression: bool=False
        ):
        ...
        # Assign orginary_regression for r2 calculation
        self.ordinary_regression = ordinary_regression

        # Get the number of classes for ordinary regression
        if self.ordinary_regression:
            self.num_classes = dataloader.dataset.num_classes
    ...
    def train_step_v2(
        ...
        for batch, (X, y) in self.progress_bar(
            ...
            if self.ordinary_regression:                
                y_pred = self.decode_ordinary_regression(y_pred, num_classes=self.num_classes).float().view(-1)
                y = self.decode_ordinary_regression(y, num_classes=self.num_classes).float().view(-1)
            else:
